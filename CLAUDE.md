...
**Subjects:** declarative, answering *why* the change is being made — the rationale, need, or constraint it serves. e.g. *"X needs Y"*, *"X requires Y"*, *"A enables B"*, *"A will enable B"*. Imperative voice (`add X`, `remove Y`) and mechanism-narrating phrases (`X directive added`, `Y section removed`) are wrong shape.
