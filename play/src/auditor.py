...
def _entries_from(failures: list[dict]) -> list[dict[str, str]]:
    return [
        {"characteristic": row["characteristic"], "failure": row["failure"]}
        for row in failures
    ]


def _failures_from(
    content: str, working_dir: Path, should: list[dict], reference_scene: Path
) -> list[dict]:
    return [
        row
        for row in should
        if not row["verify"](content, working_dir, reference_scene)
    ]


def _passes_from(should: list[dict]) -> list[dict[str, str]]:
    return [
        {"characteristic": row["characteristic"], "status": "PASS"} for row in should
    ]
