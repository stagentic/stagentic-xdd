...
| 5 | [The failing test is written before the production code — RECURRENCE](20260712-1855-write-the-failing-test-before-the-production-code-RECURRED/lesson.md) | Recurrence of #2 for no apparent reason (same config); the misstep still fired ~20% of runs — which the original 0/10 validation was too small to have ruled out. Re-corrected with a workflow, a read-first step, and motivation. Retested at 100 runs (100/100 pass rate). |
